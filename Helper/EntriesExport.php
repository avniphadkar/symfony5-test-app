...
class EntriesExport
{
    ...
    private $title = '';
    private $entries = [];
    private $author = 'testapp';
    private $language = '';

    /**
     * @param TranslatorInterface   $translator   Translator service
     * @param string                $testappUrl  TestApp instance url
     * @param string                $logoPath     Path to the logo FROM THE BUNDLE SCOPE
     * @param TokenStorageInterface $tokenStorage Needed to retrieve the current user
     */
    public function __construct(TranslatorInterface $translator, $testappUrl, $logoPath, TokenStorageInterface $tokenStorage)
    {
    ...
    private function produceEpub()
    {
        ...
        $content_start =
            ...
            . "<title>testapp articles book</title>\n"
            . "</head>\n"
            . "<body>\n";
        ...
        $book->setDescription('Some articles saved on my testapp');

        $book->setAuthor($this->author, $this->author);

        // I hope this is a non-existent address :)
        $book->setPublisher('testapp', 'testapp');
        // Strictly not needed as the book date defaults to time().
        $book->setDate(time());
        $book->setSourceURL($this->testappUrl);

        $book->addDublinCoreMetadata(DublinCore::CONTRIBUTOR, 'PHP');
        $book->addDublinCoreMetadata(DublinCore::CONTRIBUTOR, 'testapp');
        ...
        $hash = sha1(sprintf('%s:%s', $this->testappUrl, implode(',', $entryIds)));
        $book->setIdentifier(sprintf('urn:testapp:%s', $hash), EPub::IDENTIFIER_URI);
    ...
    private function producePdf()
    {
        ...
        $pdf->SetSubject('Articles via testapp');
        $pdf->SetKeywords('testapp');
